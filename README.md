...
The project functions as a physical wall clock where each armor bay represents one hour (1 through 12). Designed to hold 12 Iron Man figures, it features a replica H.O.M.E.R. panel and a central Elevator/Launch tube.
...
Raspberry Pi 4 Auto-start

To ensure the clock starts automatically on boot:
...
Each Pico runs CircuitPython. The board will automatically execute code.py in its onboard memory upon receiving power or an enable signal. Please note that different boards require different versions of CircuitPython; refer to circuitpython.org for the correct version.

📜 Credits & Attributions

3D Design: Building the 3D STL files was done using TinkerCad.

Original design/dimensions: WayGroovy's "Iron Man Hall of Armor".

Honeycomb side pattern: Remixed from ines.lopez's "panel".

Front Panel: Elements from ZDP189's "NASA Apollo Mission Control Panels".

Code & Assets:

OLED tutorials by educ8s.tv.

Arc Reactor Vector art via OnlineResize.club.
...
This project is intended for the Maker community. Individuals are welcome to sell custom builds or improve the design, but commercial use by large-scale manufacturers is not authorized without a separate agreement.
